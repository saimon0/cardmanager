...
function validateData($id, $companyName, $balance, $pin, $activationDate, $expirationDate, $creationDate)
{
    ...
    {
        $response = json_encode(array('type'=>'error','message' =>'Provided cards ID does not exists!'));
    }
    else
    {
        $response = json_encode(array('type'=>'success','message' =>'Success!'));
        updateQuery($id,$companyName,$balance,$pin,$activationDate,$expirationDate,$creationDate);
    }
    echo $response;
}


function updateQuery($id,$companyName,$balance,$pin,$activationDate,$expirationDate,$creationDate)
{
    ...
    $sql = $conn->prepare("UPDATE cards SET CompanyName= ?,Balance = ?, Pin = ?, ActivationDate = ?, ExpirationDate = ? WHERE Id='$id'");

    $sql->bind_param('sssss',$balance,$companyName,$pin,$activationDate,$expirationDate);
    echo $sql;
